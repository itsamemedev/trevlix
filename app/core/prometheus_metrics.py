...
def build_prometheus_lines(*, bot_version: str, state, db) -> list[str]:
    ...
    lines = []
    # Best-effort: a single attribute/state error must never take down /metrics.
    try:
        ...
        lines.append(f"trevlix_closed_trades_total {len(closed)}")
    ...
    except Exception as exc:  # noqa: BLE001 - state read must never break /metrics
        log.debug("prometheus state metrics failed: %s", exc)

    try:
        if db is not None and db._pool:
            lines.append(f"trevlix_db_pool_available {db._pool.available}")
            lines.append(f"trevlix_db_pool_size {db._pool.pool_size}")
    except Exception as exc:  # noqa: BLE001 - pool read must never break /metrics
        log.debug("prometheus pool metrics failed: %s", exc)
