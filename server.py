    ...
    def get_user_exchanges(self, user_id: int) -> list[dict]:
        ...
        try:
            with self._get_conn() as conn:
                with conn.cursor() as c:
                    c.execute(
                        ...
                        "CASE WHEN api_secret IS NOT NULL AND api_secret <> '' THEN 1 ELSE 0 END AS has_api_secret "
            ...
            for r in rows:
                ...
                d["has_api_secret"] = bool(d.get("has_api_secret"))
                result.append(d)
            return result
        except Exception as e:
...
def on_start_bot():
    ...
    if not _ws_rate_check("start_bot", min_interval=3.0):
        ...
        return
    is_live_mode = not CONFIG.get("paper_trading", True)
    if is_live_mode:
        ...
        api_secret = (
            decrypt_value(raw_secret.reveal() if hasattr(raw_secret, "reveal") else raw_secret)
            if raw_secret
            else ""
        )
        if not api_key or not api_secret:
            emit(
                "status",
                {
                    "msg": "❌ Live-Start abgebrochen: API-Key/Secret fehlen",
                    "key": "ws_bot_start_live_keys_missing",
                    "type": "error",
                },
            )
            state.add_activity(
                "❌",
                "Live-Start abgebrochen",
                "API-Key/Secret fehlen oder konnten nicht entschlüsselt werden",
                "error",
            )
            return
    try:
        ...
        if is_live_mode:
            emit(
                "status",
                {
                    "msg": f"❌ Live-Start fehlgeschlagen: {preflight_err}",
            ...
            state.add_activity(
                ...
                f"Exchange nicht erreichbar: {str(preflight_err)[:120]}",
                "error",
            )
            return
        # Soft-Fail in Paper-Mode: Bot trotzdem starten
        emit(
            "status",
            {
                "msg": f"⚠️ Preflight Warnung: {preflight_err} (Bot startet trotzdem)",
                "key": "ws_bot_start_preflight_warning",
                "type": "warning",
            },
        )
        state.add_activity(
            "⚠️",
            "Preflight Warnung",
            "Exchange/Marktdaten aktuell nicht verfügbar – Bot läuft weiter",
            "warning",
        )
    with state._lock:
        if state.running:
    ...
    try:
        ...
        for ex in user_exchanges:
            ...
            ex_map[ex_name] = {
                ...
                "has_api_key": bool(ex.get("has_api_key")),
                "has_api_secret": bool(ex.get("has_api_secret")),
                "portfolio_value": 0.0,
                "return_pct": 0.0,
                "trade_count": 0,
                "open_trades": 0,
