...
def bot_loop():
    ...
    def _touch_heartbeat(force: bool = False) -> None:
        nonlocal _last_hb
        now = time.monotonic()
        if not force and (now - _last_hb) < 5.0:
            return
        try:
            healer.heartbeat()
            _last_hb = now
    ...
    while state.running:
        ...
        try:
            ...
            if not regime.is_bull and CONFIG.get("use_shorts"):
                ...
                if short_candidates:
                    short_batch_timeout = max(
                        10, _safe_int(CONFIG.get("scan_batch_timeout_s", 45), 45)
                    )
                    with ThreadPoolExecutor(max_workers=CONFIG.get("max_workers", 4)) as short_pool:
                        ...
                        try:
                            for fut in as_completed(short_futures, timeout=short_batch_timeout):
                                _touch_heartbeat()
                                try:
                                    scan = fut.result(timeout=5)
                                    if (
                                        scan
                                        and scan["signal"] == -1
                            ...
                            for fut in short_futures:
                                if not fut.done():
                                    fut.cancel()
                            log.warning(
                                "Short-Scan Timeout nach %ss – verbleibende Futures abgebrochen",
                                short_batch_timeout,
                            )
            ...
            if (
                ...
                long_batch_timeout = max(10, _safe_int(CONFIG.get("scan_batch_timeout_s", 45), 45))
                with ThreadPoolExecutor(max_workers=CONFIG.get("max_workers", 4)) as pool:
                    ...
                    try:
                        for fut in as_completed(futures, timeout=long_batch_timeout):
                            _touch_heartbeat()
                            try:
                        ...
                        for fut in futures:
                            if not fut.done():
                                fut.cancel()
                        log.warning(
                            "Long-Scan Timeout nach %ss – verbleibende Futures abgebrochen",
                            long_batch_timeout,
                        )
